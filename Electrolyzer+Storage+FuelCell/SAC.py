...
try:
    ...
    # 定义可能的文件路径 (根据你的描述)
    # 代码会自动在这些路径下搜索 price_after_MAD_96.pkl
    possible_paths = [
...
def plot_results(h_levels, prices, prod_actions, fc_actions, ele_loads, init_h, algorithm_name="SAC"):
...
def train_sac():
    print("=== 开始训练 SAC Agent (7 Days, 50k Steps) ===")
    ...
    model = SAC(
        ...
        verbose=1,
        learning_rate=3e-4,
        buffer_size=50000, # 稍微调小buffer适应短训练
...
def test_sac(model):
    ...
    print(f"SAC - 最终净电费成本: ${total_net_cost:.2f}")
    print(f"SAC - 燃料电池创造价值: ${total_fc_value:.2f}")

    plot_results(h_levels, prices, prod_actions, fc_actions, ele_loads, test_env.initial_level, algorithm_name="SAC")
