...
def plot_results(h_levels, prices, prod_actions, fc_actions, ele_loads, init_h, algorithm_name="A2C"):
...
def train_a2c():
    print("=== 开始训练 A2C Agent (7 Days, 50k Steps) ===")

    env = make_vec_env(StartingCaseEnv, n_envs=4, vec_env_cls=DummyVecEnv, seed=42) # A2C通常受益于多环境并行

    model = A2C(
        "MlpPolicy",
        env,
        verbose=1,
        learning_rate=3e-5, # A2C 对学习率较为敏感
        n_steps=20, # A2C 的 n_steps 通常较小
...
def test_a2c(model):
    ...
    print(f"A2C - 最终净电费成本: ${total_net_cost:.2f}")
    print(f"A2C - 燃料电池创造价值: ${total_fc_value:.2f}")

    plot_results(h_levels, prices, prod_actions, fc_actions, ele_loads, test_env.initial_level, algorithm_name="A2C")
