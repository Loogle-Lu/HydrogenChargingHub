...
class HydrogenEnv(gym.Env):
    ...
    def reset(self):
        ...
        return self._reset_components_and_state()

    def reset_with_scenario(self, scenario_config):
        """
        v3.7: 使用指定场景配置重置环境。
        ...
        scenario_config : dict
            包含 pv_day_idx, wind_day_idx, demand_start_idx, demand_multiplier
            (由 ScenarioManager.get_scenario_config() 返回)
        ...
        """
        self.current_step = 0
        self.data_loader.reset_with_scenario(**scenario_config)
        return self._reset_components_and_state()

    def _reset_components_and_state(self):
        """内部方法: 重置所有物理组件并构建初始状态向量"""
