...
输出:
- Figure_2_exp2_compressor_reward_profit.png: 1×3 — Compressor Energy | Reward 曲线 | Profit 曲线（仍为 20 episode 滑动平均绘制）
- Figure_2_exp2_DI_bypass_C3APC.png: 1×3 — DI 动态级间冷却(制冷负荷) | IBC 旁路次数 | C3 能耗(APC 相关)
"""
...
NUM_EPISODES = 1000  # 6D 搜索空间，需要足够步骤收敛
WARMUP_STEPS = 400
BATCH_SIZE = 256
LR = 3e-4
MA_WINDOW = 20
...
def main():
    ...
    def _bar_with_labels(ax, means, stds, ylabel, title, fmt_list):
        ...
        for i, (m, s, fmt) in enumerate(zip(means, stds, fmt_list)):
            ax.text(i, m + s + max(stds) * 0.05 + 1e-6, fmt, ha="center", va="bottom", fontsize=8)

    # --- Figure 1: Compressor energy | Reward MA | Profit MA ---
    fig1, axs1 = plt.subplots(1, 3, figsize=(14, 4.2), constrained_layout=True)
    fig1.suptitle(
        "Exp2: Action Ablation — Full 6D vs Naive 2D\n"
        "Compressor Energy & Training Curves",
        fontsize=11,
        fontweight="bold",
    )
    ...
    _bar_with_labels(
        ...
        [c_full_m, c_fix_m],
        [c_full_s, c_fix_s],
        "Compressor Energy (kWh/Ep)",
        "(a) Total Compressor Energy (Lower=Better)",
        [f"{c_full_m:.0f}±{c_full_s:.0f}", f"{c_fix_m:.0f}±{c_fix_s:.0f}"],
    )

    ep_range = np.arange(MA_WINDOW - 1, len(rewards_full))
    axs1[1].plot(ep_range, moving_average(rewards_full, MA_WINDOW), color=colors[0],
                 linewidth=2, label="Full 6D")
    axs1[1].plot(ep_range, moving_average(rewards_fixed, MA_WINDOW), color=colors[1],
                 linewidth=2, label="Naive 2D")
    axs1[1].set_xlabel("Episode")
    axs1[1].set_ylabel("Reward (scaled)")
    axs1[1].set_title("(b) Reward")
    axs1[1].legend(loc="best", fontsize=8)
    axs1[1].grid(True, alpha=0.3, linestyle="--")

    axs1[2].plot(ep_range, moving_average(profits_full, MA_WINDOW), color=colors[0],
                 linewidth=2, label="Full 6D")
    axs1[2].plot(ep_range, moving_average(profits_fixed, MA_WINDOW), color=colors[1],
                 linewidth=2, label="Naive 2D")
    axs1[2].set_xlabel("Episode")
    axs1[2].set_ylabel("Profit ($)")
    axs1[2].set_title("(c) Profit")
    axs1[2].legend(loc="best", fontsize=8)
    axs1[2].grid(True, alpha=0.3, linestyle="--")

    name1 = "Figure_2_exp2_compressor_reward_profit.png"
    plt.savefig(name1, dpi=150, bbox_inches="tight")
    plt.close(fig1)

    # --- Figure 2: DI (chiller) | IBC bypass | C3/APC energy ---
    fig2, axs2 = plt.subplots(1, 3, figsize=(14, 4.2), constrained_layout=True)
    fig2.suptitle(
        "Exp2: DI (DRL-controlled) | IBC Bypass | APC (C3 Stage Energy)",
        fontsize=11,
        fontweight="bold",
    )
    ...
    _bar_with_labels(
        ...
        [ch_full_m, ch_fix_m],
        [ch_full_s, ch_fix_s],
        "Chiller Electrical Load (kWh/Ep)",
        "(a) DI — Dynamic Intercooling\n(c₁/c₂ cooling intensity → chiller load, Lower=Better)",
        [f"{ch_full_m:.1f}±{ch_full_s:.1f}", f"{ch_fix_m:.1f}±{ch_fix_s:.1f}"],
    )
    ...
    _bar_with_labels(
        ...
        [bp_full_m, bp_fix_m],
        [bp_full_s, bp_fix_s],
        "IBC Activations / Ep (Last 20)",
        "(b) Intelligent Bypass Control (IBC)\n(Higher ⇒ more bypass, less compression)",
        [f"{bp_full_m:.1f}±{bp_full_s:.1f}", f"{bp_fix_m:.1f}±{bp_fix_s:.1f}"],
    )
    ...
    _bar_with_labels(
        ...
        [c3_full_m, c3_fix_m],
        [c3_full_s, c3_fix_s],
        "C3 Compressor Energy (kWh/Ep)",
        "(c) APC — Stage-3 Energy\n(c₃_pressure_bias; Naive fixed at max)",
        [f"{c3_full_m:.1f}±{c3_full_s:.1f}", f"{c3_fix_m:.1f}±{c3_fix_s:.1f}"],
    )

    name2 = "Figure_2_exp2_DI_bypass_C3APC.png"
    plt.savefig(name2, dpi=150, bbox_inches="tight")
    plt.close(fig2)
