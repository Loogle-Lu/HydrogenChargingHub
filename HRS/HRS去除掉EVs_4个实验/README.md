...
- **统一级联充装原则 (两种车型遵循相同物理定律)**
  ...
  - **动态压力门控**：SOG 边界 = `(max_pressure × SOC - min_dp) / target_pressure`
...
- **动态压力示例**
  - T3₂(350 bar) SOC=0.7 → 实际压强=245 bar → 只能服务 HDV SOG < (245-5)/350 = 0.686
  - T3₂(350 bar) SOC=0.4 → 实际压强=140 bar → 只能服务 HDV SOG < (140-5)/350 = 0.386
  - 这迫使 RL Agent 学习保持储罐高 SOC，否则面临服务能力下降和缺氢惩罚

- **FCEVServiceStation 增强**
  - 新增 `current_fcev_350_avg_sog` 追踪，两类车型 SOG 均可观测
  - SOG 信息传递至 `step_all()` 用于差压门控计算
...
- **数据流变更**
  ...
  - 新增 `_compute_dynamic_sog_boundaries()` 统一计算动态 SOG 边界
  - info dict 新增 `c3_booster_flow`, `avg_sog_350`, `avg_sog_700`
