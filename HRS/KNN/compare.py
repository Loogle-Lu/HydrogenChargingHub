...
- Reward 曲线
- Average Profit 曲线
- (v3.7 可选) Scenario-based Robustness Test
...
# v3.7: 场景鲁棒性测试配置
ENABLE_SCENARIO_TEST = True   # 训练后是否运行场景评估
SCENARIO_EVAL_EPISODES = 5    # 每个场景评估的 Episode 数
...
def run_scenario_robustness_test(trained_agents, enable_i2s=True):
    """
    v3.7: 训练完成后对所有算法做 Scenario-based 鲁棒性快速评估。

    Parameters
    ----------
    trained_agents : dict[algo_name] = agent
    enable_i2s : bool
    """
    ...
    print("\n" + "=" * 70)
    print("  [v3.7] SCENARIO ROBUSTNESS TEST")
    print("=" * 70)

    dl = DataLoader()
    sm = ScenarioManager(dl)
    scenario_configs = sm.get_all_scenarios()
    scenario_names = sm.get_all_scenario_names()

    # 评估
    eval_results = {}
    for algo_name in ALGO_NAMES:
        eval_results[algo_name] = {}
        agent = trained_agents[algo_name]

        for scene_name in scenario_names:
            cfg = scenario_configs[scene_name]
            env = HydrogenEnv(enable_i2s_constraint=enable_i2s)
            rewards = []
            profits = []

            for ep in range(SCENARIO_EVAL_EPISODES):
            ...
            eval_results[algo_name][scene_name] = {
    ...
    # 打印简表
    col_w = 12
    sep_len = 20 + col_w * len(ALGO_NAMES)
    header = f"{'Scenario':<20s}" + "".join(f"{n:>{col_w}s}" for n in ALGO_NAMES)

    for metric, key in [("Avg Reward", "avg_reward"), ("Avg Profit", "avg_profit")]:
        print(f"\n  --- {metric} by Scenario ---")
        print(header)
        print("-" * sep_len)
        for scene in scenario_names:
            ...
            for algo in ALGO_NAMES:
                v = eval_results[algo][scene][key]
                line += f"{v:{col_w}.2f}"
            print(line)
        print()

    # 稳定性排名 (基于跨场景变异系数)
    print("  --- Robustness Ranking (lower CV = more stable) ---")
    cv_list = []
    for algo in ALGO_NAMES:
        vals = [eval_results[algo][s]["avg_reward"] for s in scenario_names]
        avg = np.mean(vals)
        std = np.std(vals)
        cv = std / max(abs(avg), 1e-6)
        cv_list.append((algo, avg, cv))
    cv_list.sort(key=lambda x: (-x[1], x[2]))
    for rank, (algo, avg_r, cv) in enumerate(cv_list, 1):
        tag = "Stable" if cv < 0.3 else "Moderate" if cv < 0.6 else "Unstable"
        print(f"  #{rank} {algo:<6s} Avg Reward: {avg_r:>8.2f}, CV: {cv:.4f} ({tag})")
    print("=" * 70)
...
def main():
    ...
    if ENABLE_SCENARIO_TEST:
        print(f"  Scenario Test:          Enabled ({SCENARIO_EVAL_EPISODES} eps/scenario)")
    print("=" * 70)

    results_by_condition = {}
    last_trained_agents = {}  # v3.7: 保存最后一轮训练的 agents
    ...
    # --- v3.7: 场景鲁棒性测试 ---
    if ENABLE_SCENARIO_TEST and last_trained_agents:
        run_scenario_robustness_test(last_trained_agents, enable_i2s=False)
