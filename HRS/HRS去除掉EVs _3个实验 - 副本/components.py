...
class Electrolyzer:
    ...
    def compute(self, power_input, re_available=0.0):
        ...
        # η = H2化学能 / 输入电能 → h2 (kg/h) = power (kW) × η / H2_LHV (kWh/kg)
        h2_flow_kg = power * self.efficiency / Config.H2_LHV
...
class MultiStageCompressorSystem:
    ...
    C3: ≤500 bar → ≤850 bar (T3 在线取气直充 FCEV; v6.0 APC + SAE J2601 护栏)
    ...
    def _compute_bypass_ratio(
        self,
        downstream_pressure,
        target_stage_pressure,
        demand,
        bypass_bias=None,
        upstream_pressure=None,
        use_upstream_for_pressure_factor=False,
    ):
        """
        连续 IBC [0, 1]: bypass_bias × pressure_factor × demand_factor × dp_factor.

        - C1/C2: pressure_factor 用 下游/本级目标压；dp = 上游 − 下游。
        - C3: pressure_factor 用 上游 T3 / APC 目标压 (与原 tank/target 语义一致)；
              dp = T3₃ − 车内压 (自然旁路须有足够正压差)。
        若未传 upstream_pressure，则 dp_factor=1（仅不推荐用于 C1/C2/C3 主路径）。
        """
        ...
        if upstream_pressure is None:
            ...
            dp = float(upstream_pressure) - float(downstream_pressure)
            if dp <= 0:
                dp_factor = 0.0
            else:
                bias = np.clip(bypass_bias, 0.0, 1.0)
                min_dp = (
                    Config.bypass_min_dp_conservative
                    - (Config.bypass_min_dp_conservative - Config.bypass_min_dp_aggressive) * bias
                )
                ramp = max(getattr(Config, "bypass_dp_ramp_bar", 10.0), 1e-6)
                dp_factor = float(np.clip((dp - min_dp) / ramp, 0.0, 1.0))

        if target_stage_pressure > 0:
            if use_upstream_for_pressure_factor and upstream_pressure is not None:
                pr = float(upstream_pressure) / float(target_stage_pressure)
            else:
                pr = float(downstream_pressure) / float(target_stage_pressure)
        ...
        return float(np.clip(
            bypass_bias * pressure_factor * demand_factor * dp_factor, 0.0, 1.0))
    
    def _compute_adaptive_target_pressure(self, avg_fcev_soc, c3_pressure_bias=None):
        """
        v6.0 三级判定 APC:
          L1 查表 + DRL 缩放 → L2 SAE J2601 护栏 (≥车内压+min_diff) → L3 min(·, c3_output_pressure)
        """
        ...
        if c3_pressure_bias is not None:
            ...
            drl_target = base * scale
        else:
            drl_target = base

        current_fcev_pressure = avg_fcev_soc * Config.fcev_target_pressure
        min_required = current_fcev_pressure + Config.sae_j2601_min_pressure_diff
        safe_target = max(drl_target, min_required)
        final_target = min(safe_target, Config.c3_output_pressure)
        return float(final_target)
    
    def compute_c1(self, mass_flow, tank_pressure=0, electricity_price=0.08, cooling_intensity=None,
                   bypass_bias=None, upstream_pressure=None):
        """
        C1压缩: T1(2bar) -> T2(35bar)
        v6.0: 连续旁路 + 压差门控 (上游 T1 → 下游 T2)
        """
    ...
    def compute_c2(self, mass_flow, tank_pressure=0, electricity_price=0.08, cooling_intensity=None,
                   bypass_bias=None, upstream_pressure=None):
        """
        C2压缩: T2(35bar) -> T3系统(500bar)
        v6.0: 连续旁路 + 压差门控 (上游 T2 管线 → 下游 T3 等效压)
        """
        bypass_ratio = self._compute_bypass_ratio(
    ...
    def compute_c3(self, mass_flow, avg_fcev_soc=0.5, tank_pressure=0, electricity_price=0.08,
                   bypass_bias=None, c3_pressure_bias=None, downstream_fcev_pressure=None):
        """
        C3: T3₃ → FCEV; v6.0 APC + 连续旁路 (上游 T3₃ → 下游车内压)
        """
        target_pressure = self._compute_adaptive_target_pressure(avg_fcev_soc, c3_pressure_bias)
        if downstream_fcev_pressure is None:
            downstream_fcev_pressure = avg_fcev_soc * Config.fcev_target_pressure
        bypass_ratio = self._compute_bypass_ratio(
            downstream_fcev_pressure,
            target_pressure,
            mass_flow,
            bypass_bias,
            upstream_pressure=tank_pressure,
            use_upstream_for_pressure_factor=True,
        )
    ...
    def compute_total_power(self, flow_c1, flow_c2, flow_c3, 
        ...
        if tank_pressures is None:
            tank_pressures = {'t1': 0, 't2': 0, 't3': 0, 't3_3': 0, 'fcev_p': 0}

        p1, h1 = self.compute_c1(
            flow_c1, tank_pressures.get('t2', 0), electricity_price,
            upstream_pressure=tank_pressures.get('t1'))
        p2, h2 = self.compute_c2(
            flow_c2, tank_pressures.get('t3', 0), electricity_price,
            upstream_pressure=tank_pressures.get('t2_up', tank_pressures.get('t2')))
        fcev_p = tank_pressures.get('fcev_p')
        if fcev_p is None:
            fcev_p = avg_fcev_soc * Config.fcev_target_pressure
        p3, h3 = self.compute_c3(
            flow_c3,
            avg_fcev_soc=avg_fcev_soc,
            tank_pressure=tank_pressures.get('t3_3', 0),
            electricity_price=electricity_price,
            downstream_fcev_pressure=fcev_p,
        )
...
class FuelCell:
    ...
    def compute(self, h2_input_kg):
        # η = 输出电能 / H2化学能 → power (kW) = h2 (kg/h) × H2_LHV (kWh/kg) × η
        power_generated = h2_input_kg * Config.H2_LHV * self.efficiency
        actual_power = min(power_generated, self.max_power)
        consumed_h2 = actual_power / (Config.H2_LHV * self.efficiency)
