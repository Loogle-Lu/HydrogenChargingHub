...
class Config:
    # --- 路径配置 ---
    # Linux/SSH 服务器上的真实原始数据路径；DataLoader 会基于这些数据再加入随机噪声。
    path_price = "/home/s4212681/HydrogenChargingHub/price_after_MAD_96.pkl"
    path_pv = "/home/s4212681/HydrogenChargingHub/pv_power_100.pkl"
    path_wind = "/home/s4212681/HydrogenChargingHub/wd_power_150.pkl"
    ...
    # --- 物理组件参数 ---
    # 氢气低热值 (Lower Heating Value) — 统一效率换算基准
    H2_LHV = 33.3  # kWh/kg

    # 1. 电解槽
    ele_max_power = 1000.0  # kW
    ele_efficiency = 0.80  # 电解效率 80% (η = H2化学能 / 输入电能, 基于H2_LHV)
    ...
    # v3.1 / 连续 IBC: 智能旁路 + 物理压差门控 (上游→下游须有足够正压差才可能旁路)
    enable_bypass = True  # 启用旁路控制
    bypass_min_dp_conservative = 15.0  # bar (bypass_bias→0: 保守, 需更大压差)
    bypass_min_dp_aggressive = 2.0   # bar (bypass_bias→1: 积极)
    # 超过 min_dp 后，再增加多少 bar 旁路因子趋于 1（连续 IBC 平滑）
    bypass_dp_ramp_bar = 10.0
    bypass_demand_threshold = 5.0  # kg/h，需求低于此值时更易旁路
    ...
    cooling_power_kw = 300.0      # 略提高：电价高时乱开冷却更亏，利于离线SAC学「价-冷」联动
    
    # v6.0: 压力自适应控制 (APC) — 三级判定
    # ① 查表 + DRL(c3_pressure_bias) 缩放 → ② SAE J2601 护栏(≥车内压 + min_diff)
    # ③ 设备上限 = c3_output_pressure (850 bar)
    ...
    c3_output_pressure = 850.0  # bar (v6.0: C3/APC 物理天花板，与 J2601 高压工况一致)
    c3_max_flow = 15.0  # kg/h

    # v6.0: SAE J2601 rapid fueling — C3 目标压强下限相对车内压的最小裕量 (bar)
    sae_j2601_min_pressure_diff = 50.0
    ...
    fill_heat_penalty = 0.30          # 废热负载每单位对完成率的惩罚 (↓ from 0.35)
    ...
    # 4. 燃料电池
    fc_max_power = 500.0
    fc_efficiency = 0.60  # 燃料电池效率 60% (η = 输出电能 / H2化学能, 基于H2_LHV)
    ...
    # 氢转电: 1 kg H2 → H2_LHV × fc_efficiency = 33.3×0.60 = 20 kWh, 成本≈制氢电费; 电网: 购价=电价
    # 合理区间: 0.35~0.45 $/kWh
