...
**v6.0 (2026-03-28) - 连续旁路控制 + 全因子消融 + 移除 I2S 约束**

三项重大变更，涉及 `components.py`、`env.py`、`config.py`、`exp2_technologies_ablation.py` 及所有实验/对比脚本。

- **旁路控制: 二值 → 连续 (`components.py`)**
  - 移除 `_check_bypass()` (返回 True/False 的二值判断)
  - 新增 `_compute_bypass_ratio()` 返回连续值 [0, 1]，公式: `bypass_ratio = bypass_bias × pressure_factor × demand_factor`
    - `bypass_bias`: RL 动作，控制旁路积极程度
    - `pressure_factor`: 储罐压力 / 目标压力在 [0.6, 0.9] 线性映射到 [0, 1]
    - `demand_factor`: 需求越低旁路越可行，2× threshold 处降为 0
  - `compute_c1/c2/c3` 改为: 先计算满功率 `full_power`，实际功耗 = `(1 - bypass_ratio) × full_power`
  - 物理意义: 对应部分开启的旁路阀，为 SAC 提供平滑梯度信号
  - `bypass_activations` 统计改为累计连续比例 (float)，不再是整数计数

- **实验1 重构: 全因子递增消融 (`exp2_technologies_ablation.py`)**
  - 实验设计从 Subtractive Ablation (逐项去除, 6 组) 改为 **Additive Ablation** (递增组合, 16 组)
  - 4 项技术 (VSD, DI, BP, AP) 的全部 C(4,k) 组合 (k=0..4)，共 2^4 = 16 组
  - 按技术数量分 5 层: L0(Naive) → L1(+1 tech, 4 组) → L2(+2 techs, 6 组) → L3(+3 techs, 4 组) → L4(Full)
  - 绘图: 柱状图按层分组，用不同色系区分；学习曲线每层选最佳代表
  - 末尾输出完整汇总表

- **完全移除 I2S (Identical Initial State) 约束**
  - `config.py`: 删除 `enable_i2s_constraint`、`i2s_penalty_weight` 参数
  - `env.py`: 删除 `__init__` 的 `enable_i2s_constraint` 参数及回合末 I2S 终端惩罚计算
  - `exp1_cascade_ablation.py`: 删除 `EXP3_I2S` 变量，`NaiveArchEnv.__init__` 不再接受 I2S 参数
  - `exp2_technologies_ablation.py`、`exp3_actions_ablation.py`: `HydrogenEnv()` 不再传 I2S 参数
  - `compare.py`: 移除所有 "With I2S" 标题/注释
  - `Transformer Test.py`: 移除所有 I2S 相关注释和标题

---

**v5.2 (2026-03-24) - 训练数据与 `plot_data` 合成规则对齐**

- **`data_loader.py`**：电价 / PV / 风电不再直接使用 pickle 原始逐日索引，改为与 **`plot_data.py` 同款** 生成 **365×96** 对齐序列：电价 = 日曲线平铺 + **独立高斯噪声**（σ=0.03）+ clip；PV/WD = **随机日拼接** × **自相关乘性噪声**（AR(1)+exp，ρ_pv=0.9、ρ_wd=0.8）。
- **`reset()`**：在全年合成序列上 **随机滑动 96 步窗口**（每 episode 一段连续步，与电价/PV/WD 时间对齐）。
- **`plot_data.py`**：改为调用 `data_loader.synthesize_series`，保证成像与训练一致。
...
- **统一级联充装原则 (两种车型遵循相同物理定律)**
  - 核心规则：**始终从压力最低的可用储罐开始充装**，利用压差自然驱动氢气流入车辆
  - **动态压力门控**：SOC 边界 = `(max_pressure × SOC - min_dp) / target_pressure`
    储罐实际压强随气体消耗而下降，门控值实时跟随，无需固定切换点
  - 最低压差阈值 `min_cascade_pressure_diff = 5 bar`：压差不足时该罐不参与充装
  - 论文叙事统一：一个原则解释所有充装行为，不再出现互相矛盾的取气顺序

- **350-bar HDV 充装流程 (三罐级联直充)**
  - Phase 1: T3₁ → 车辆 (实际压强 200×SOC > 车辆背压 + 5 bar)
  - Phase 2: T3₂ → 车辆 (实际压强 350×SOC > 车辆背压 + 5 bar)
  - Phase 3: T3₃ → 车辆 (T3₂ 压力不足时由 T3₃ 补充顶充)
  - 各相 SOC 边界随储罐 SOC 动态变化，非固定值

- **700-bar LDV 充装流程 (级联直充 + C3 增压末段)**
  - Phase 1-3: T3₁→T3₂→T3₃ 直充 (动态压力门控)
  - Phase 4: T3₃ → C3 → 700 bar → 车辆 (级联直充覆盖不到的增压部分)
  - C3 工作量由储罐实际状态决定，储罐 SOC 越高→直充覆盖范围越大→C3 工作越少

- **C3 角色变更：从"全量压缩"改为"末段增压泵"**
  - 旧模型：所有 700-bar 需求 100% 经 C3
  - 新模型：仅级联直充覆盖不到的部分经 C3，比例随储罐 SOC 动态变化
  - C3 流量由 `step_all()` 根据实际压力分配后返回，不再由 env 预设

- **动态压力示例**
  - T3₂(350 bar) SOC=0.7 → 实际压强=245 bar → 只能服务 HDV SOC < (245-5)/350 = 0.686
  - T3₂(350 bar) SOC=0.4 → 实际压强=140 bar → 只能服务 HDV SOC < (140-5)/350 = 0.386
  - 这迫使 RL Agent 学习保持储罐高 SOC，否则面临服务能力下降和缺氢惩罚

- **FCEVServiceStation 增强**
  - 新增 `current_fcev_350_avg_soc` 追踪，两类车型 SOC 均可观测
  - SOC 信息传递至 `step_all()` 用于差压门控计算

- **Config 新增**
  - `min_cascade_pressure_diff = 5.0` (bar): 级联充装最低压差阈值

- **数据流变更**
  - `step_all()` 返回值新增 `c3_flow` (实际需要 C3 增压的流量)
  - `_compute_comp_block()` 不再包含 C3，C3 在储罐更新后单独计算
  - 新增 `_compute_dynamic_soc_boundaries()` 统一计算动态 SOC 边界
  - info dict 新增 `c3_booster_flow`, `avg_soc_350`, `avg_soc_700`
...
- **H₂ 取气顺序修正（同版本，v4.5 再次修正为统一 Low→High）**
  - ~~350-bar 车：T3₂(350 bar) 优先 → T3₁(200 bar) 补充~~ → v4.5 改为 T3₁→T3₂
  - ~~700-bar 车：T3₃ → T3₂ → T3₁ 级联供 C3~~ → v4.5 改为 T3₁→T3₂→T3₃ 直充 + C3 增压末段
...
**v4.1 (2026-02-19) - 卖氢主导 + EV优先氢转电 + 电网成本精细化**

- **卖氢收益 >> 卖电收益**
  - `fcev_service_price`: 12 → 18 $/kg，卖氢为主要利润源
  - `local_grid_feedin_ratio`: 0.6 → 0.42，地电网收购价更低
  - `max_grid_revenue_ratio`: 0.5 → 0.35，售电收入占比进一步压低

- **EV 供电优先级：氢转电优先，不足再购电**
  - EV 充电电力分配：优先 FC(H2转电) + 绿电，不足部分从电网购电
  - 保证氢转电利润 > 电网购电卖EV（FC 边际成本低，电网=电价）
  - 新增 info: `ev_from_fc_re`, `ev_from_grid` 供分析

- **智能体目标：FCEV 与 EV 之间 H2 分配利润最大化**
  - FCEV 加氢单价高，H2 有限时优先满足 FCEV
  - EV 充电优先用自有电（FC+RE），减少高价购电

- **电网成本模型修正**
  - 仅对实际购电量计费：电解槽 `power_from_grid` + 母线侧购电
  - 不再对整条制氢链全额计费，更贴合物理流
...
- **技术4: 自适应压力控制 (Adaptive Pressure Control)**
  ...
  **解决方案**: 根据车辆氢罐的饱和度(SOC)，分段调整充装压力，刚开始用高压快充，快满时降压慢充。
...
└────────────────────────────────────┘
    ↓ 氢气
T1 → C1(VSD+旁路) → T2 → C2(VSD+旁路) → T3₁/T3₂/T3₃
                                              ↓ Low→High 差压级联直充
                                         HDV(350bar): T3₁→T3₂ 直充
                                         LDV(700bar): T3₁→T3₂→T3₃ 直充 + T3₃→C3(APC)→700bar
```
...
自适应压力充装:
```
充装阶段1 (SOC 0-30%): 700bar快速充装
充装阶段2 (SOC 30-60%): 500bar中速充装
充装阶段3 (SOC 60-80%): 350bar慢速充装
充装阶段4 (SOC 80%+): 200bar涓流充电
```
...
├── compare.py             # 七算法对比脚本 (训练+可视化) [v3.4/v3.9]
├── env.py                  # 氢电集成环境
