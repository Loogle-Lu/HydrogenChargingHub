...
**v5.3 (2026-03-17) - IBC 物理重构：正压差驱动旁路**

- **物理判据重写**：IBC 触发条件从「下游罐压 ≥ 目标压 × 阈值」改为 **「上游罐压 > 下游罐压 + min\_dp」**，符合气体只能从高压自然流向低压的热力学事实
- **参数替换**：
  - 删除 `bypass_pressure_threshold`（旧式百分比阈值）
  - 新增 `bypass_min_dp_conservative = 15 bar`（bypass\_bias=0, 保守）和 `bypass_min_dp_aggressive = 2 bar`（bypass\_bias=1, 积极）
  - `bypass_bias` 语义：连续控制最小压差阈值，值越大→所需压差越小→更频繁旁路
- **上下游压力传递**：
  - C1：upstream = T1 有效压 (2 × SOC)，downstream = T2 有效压 (35 × SOC)
  - C2：upstream = T2 有效压，downstream = T3 三罐最低有效压
  - C3：upstream = T3₃ 有效压，downstream = FCEV 背压 (SOC × 700)
- **影响文件**：`config.py`, `components.py` (`_check_bypass`, `compute_c1/c2/c3`), `env.py` (`_compute_comp_block`, `_compute_c3_block`)
