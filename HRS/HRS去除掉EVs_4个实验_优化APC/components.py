...
class MultiStageCompressorSystem:
    ...
    C3: ≤500 bar → ≤850 bar (T3 在线取气直充 FCEV, 无 T4 缓冲; v6.0 SAE J2601 三级APC)
    ...
    def _check_bypass(self, upstream_pressure, downstream_pressure, demand, bypass_bias=None):
        """
        v5.3 IBC 物理判据:
          1. 上游罐压 > 下游罐压 (正压差, 气体才能自然流过旁路阀)
          2. 压差 ≥ min_dp (bypass_bias 控制: 积极→小阈值, 保守→大阈值)
          3. 流量 ≤ bypass_demand_threshold (小流量时才允许旁路)
        """
        ...
        if bypass_bias is not None:
            bias = np.clip(bypass_bias, 0, 1)
            min_dp = Config.bypass_min_dp_conservative - \
                     (Config.bypass_min_dp_conservative - Config.bypass_min_dp_aggressive) * bias
        else:
            min_dp = (Config.bypass_min_dp_conservative + Config.bypass_min_dp_aggressive) / 2.0

        return dp >= min_dp and demand <= Config.bypass_demand_threshold
    
    def _compute_adaptive_target_pressure(self, avg_fcev_soc, c3_pressure_bias=None):
        """
        v6.0 三级判定 APC (SAE J2601 合规):

          Level 1 — 查表 + DRL 缩放 (保留 AI 意图):
            base  = adaptive_pressure_map(avg_fcev_soc)
            scale = 0.6 + 0.8 × c3_pressure_bias
            drl_target = base × scale

          Level 2 — SAE J2601 护栏 (物理底线):
            min_required = current_fcev_pressure + 50 bar
            safe_target  = max(drl_target, min_required)

          Level 3 — 设备上限截断 (天花板):
            final_target = min(safe_target, 850)
        """
        # --- Level 1: 查表 + DRL 缩放 ---
        ...
        min_required_pressure = current_fcev_pressure + Config.sae_j2601_min_pressure_diff
        safe_target = max(drl_target, min_required_pressure)
    ...
    def compute_c1(self, mass_flow, tank_pressure=0, electricity_price=0.08,
                   cooling_intensity=None, bypass_bias=None, upstream_pressure=0):
        """
        C1压缩: T1(2bar) -> T2(35bar)
        IBC: upstream=T1有效压, downstream=T2有效压(tank_pressure)
        """
        if Config.enable_bypass and self._check_bypass(
                upstream_pressure, tank_pressure, mass_flow, bypass_bias):
    ...
    def compute_c2(self, mass_flow, tank_pressure=0, electricity_price=0.08,
        ...
        IBC: upstream=T2有效压(upstream_pressure), downstream=T3最低有效压(tank_pressure)
        """
        if Config.enable_bypass and self._check_bypass(
                upstream_pressure, tank_pressure, mass_flow, bypass_bias):
    ...
    def compute_c3(self, mass_flow, avg_fcev_soc=0.5, tank_pressure=0, electricity_price=0.08,
                   bypass_bias=None, c3_pressure_bias=None, downstream_pressure=0):
        """
        C3压缩: T3₃ -> FCEV (700bar)
        IBC: upstream=T3₃有效压(tank_pressure), downstream=FCEV背压(downstream_pressure)
        """
        target_pressure = self._compute_adaptive_target_pressure(avg_fcev_soc, c3_pressure_bias)

        if Config.enable_bypass and self._check_bypass(
                tank_pressure, downstream_pressure, mass_flow, bypass_bias):
